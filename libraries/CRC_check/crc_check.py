...
def Append_CRC8_Check_Sum(message,length):
	if ((message == 0) or (length <= 2)) :
		return;
	CRC8 = Get_CRC8_Check_Sum ( message, length-1, CRC8_INIT);
...
def Append_CRC16_Check_Sum(message,length):
	if ((message == 0) or (length <= 2)):
		return 
	CRC16 = Get_CRC16_Check_Sum (message, length-2, CRC16_INIT );
	message[length-2] = (CRC16 & 0x00ff);
	message[length-1] = ((CRC16 >> 8)& 0x00ff);
